...
- **UX 決定** (2026-07-21):
  ...
  - アプリ認証は **device_tokens + Bearer**
  - 本番の Cloudflare Access は **Service Token**
  - m5deck と API 開発は **並行**
...
- 本番では `CF-Access-Client-Id` と `CF-Access-Client-Secret` も送信する。
...
#### `GET /api/device/photos/*`

- M5用96pxサムネイルだけを返す。
- Device Tokenの所有者とR2キーのユーザー部分が一致する場合だけ取得できる。
...
- 本番では Cloudflare Access Service Tokenの2ヘッダも追加
